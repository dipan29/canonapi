...
public static class QueryHelper
{
    public static string qryMatchedResultCount = "SELECT SUM(tot.DR0) AS Kaggle_DR0, SUM(tot.DR0) AS Sushrut_DR0, SUM(tot.DR1) AS Kaggle_DR1, SUM(tot.DR1) AS Sushrut_DR1, SUM(tot.DR2) AS Kaggle_DR2, SUM(tot.DR2) AS Sushrut_DR2, SUM(tot.DR3) AS Kaggle_DR3, SUM(tot.DR3) AS Sushrut_DR3, SUM(tot.DR4) AS Kaggle_DR4, SUM(tot.DR4) AS Sushrut_DR4 FROM"
        + " ("
            + "SELECT COUNT(drlevel_kaggle) AS DR0, 0 AS DR1, 0 AS DR2, 0 AS DR3, 0 AS DR4 FROM images WHERE drlevel_kaggle = drlevel_sushrut AND drlevel_sushrut = 0"
            + " UNION ALL "
            + "SELECT 0 AS DR0, COUNT(drlevel_kaggle) AS DR1, 0 AS DR2, 0 AS DR3, 0 AS DR4 FROM images WHERE drlevel_kaggle = drlevel_sushrut AND drlevel_sushrut = 1"
            + " UNION ALL "
            + "SELECT 0 AS DR0, 0 AS DR1, COUNT(drlevel_kaggle) AS DR2, 0 AS DR3, 0 AS DR4 FROM images WHERE drlevel_kaggle = drlevel_sushrut AND drlevel_sushrut = 2"
            + " UNION ALL "
            + "SELECT 0 AS DR0, 0 AS DR1, 0 AS DR2, COUNT(drlevel_kaggle) AS DR3, 0 AS DR4 FROM images WHERE drlevel_kaggle = drlevel_sushrut AND drlevel_sushrut = 3"
            + " UNION ALL "
            + "SELECT 0 AS DR0, 0 AS DR1, 0 AS DR2, 0 AS DR3, COUNT(drlevel_kaggle) AS DR4 FROM images WHERE drlevel_kaggle = drlevel_sushrut AND drlevel_sushrut = 4"
        + ") AS tot;";
